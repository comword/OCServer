...
DLL_PUBLIC int evt_ctx_init( evt_ctx_t *tls );

/*configure the tls state machine
This apart from configuring state machine also set up cert and key */
DLL_PUBLIC int evt_ctx_init_ex( evt_ctx_t *tls, const char *crtf, const char *key );

/* set the certifcate and key in orderi. This need more breakup */
DLL_PUBLIC int evt_ctx_set_crt_key( evt_ctx_t *tls, const char *crtf, const char *key );

/* test if the certificate is set*/
DLL_PUBLIC int evt_ctx_is_crtf_set( evt_ctx_t *t );

/* test if the key is set */
DLL_PUBLIC int evt_ctx_is_key_set( evt_ctx_t *t );

/*get a new async tls endpoint from tls engine */
DLL_PUBLIC evt_tls_t *evt_ctx_get_tls( evt_ctx_t *d_eng );
...
*/
DLL_PUBLIC void evt_ctx_set_writer( evt_ctx_t *ctx, net_wrtr my_writer );
DLL_PUBLIC void evt_ctx_set_reader( evt_ctx_t *ctx, net_rdr my_reader );
DLL_PUBLIC void evt_ctx_set_nio( evt_ctx_t *ctx, net_rdr my_reader, net_wrtr my_writer );

/*clean up the resources held by async tls engine, This also closes endpoints
if any left */
DLL_PUBLIC void evt_ctx_free( evt_ctx_t *ctx );
...
DLL_PUBLIC int evt_tls_feed_data( evt_tls_t *c, void *data, int sz );

/*set up the writer and reader for this particular endpoint*/
DLL_PUBLIC void evt_tls_set_writer( evt_tls_t *tls, net_wrtr my_writer );
DLL_PUBLIC void evt_tls_set_reader( evt_tls_t *tls, net_rdr my_reader );

/*Check if handshake is over, return 1 if handshake is done otherwise 0 */
DLL_PUBLIC int evt_tls_is_handshake_over( const evt_tls_t *evt );
...
1 otherwise */
DLL_PUBLIC int evt_tls_connect( evt_tls_t *con, evt_handshake_cb cb );

/*Perform a handshake for server role endpoint, equivalent of `SSL_accept`
Upon completion, `evt_handshake_cb is called, status == 0 for failure and
1 otherwise */
DLL_PUBLIC int evt_tls_accept( evt_tls_t *tls, evt_handshake_cb cb );


/*Perform wrapping of text and do network write, `evt_write_cb` is called on
completion and status is used for status */
DLL_PUBLIC int evt_tls_write( evt_tls_t *c, void *msg, int str_len, evt_write_cb on_write );
...
DLL_PUBLIC evt_endpt_t evt_tls_get_role( const evt_tls_t *t );

/* set role to endpoint either server role or client role */
DLL_PUBLIC void evt_tls_set_role( evt_tls_t *t, enum evt_endpt_t role );

/*Gives the ptr to SSL_CTX usable raw openSSL programming */
DLL_PUBLIC SSL_CTX *evt_get_SSL_CTX( const evt_ctx_t *ctx );

/*Gives the ssl usable for doing raw OpenSSL programming */
DLL_PUBLIC SSL *evt_get_ssl( const evt_tls_t *tls );


/*check if incoming data is TLS clientHello.
return 1 if the stream is TLS and 0 otherwise
*/
DLL_PUBLIC int evt_is_tls_stream( const char *bfr, const ssize_t nrd );
